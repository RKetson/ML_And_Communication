...
# ============================================================================================ #
parser = argparse.ArgumentParser(description="Treinamento BMI Autoencoder 7,4")
...
fig.suptitle(f'Desempenho: Autoencoder BMI (7,4)', fontsize=16, fontweight='bold')
...
for ax, metric_idx, ylabel, title in [
    ...
    ax.semilogy(ebno_dbs, ref, 'k--', label='Hamming (7,4) BPSK')
    ...
    ax.semilogy(ebno_dbs, values, 'b-o', label='Autoencoder BMI (a=4)')
...
plt.title('Matriz de Distâncias Euclidianas: BMI (Autoencoder 7,4)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
plt.scatter(pca_symbols[:, 0], pca_symbols[:, 1], c='b', s=50)
for i, label in enumerate(labels):
    plt.annotate(label, (pca_symbols[i, 0], pca_symbols[i, 1]), textcoords="offset points", xytext=(0, 5), ha='center')
plt.grid(True)
plt.title('Projeção t-SNE 2D da Constelação 7D: BMI (Bit-wise)')
...
print(f"Treinamento e visualização BMI concluídos. Arquivos gerados em {FIG_DIR}")
